...
class RoleSessionsTab(QWidget):
    """User risk matrices separated into one card/table per role."""
    ...
    def _role_table(self, role, rows):
        ...
        table.setHorizontalHeaderLabels(
            ["User", "IF", "OC-SVM", "Risk Score", "Level", "Avg Files", "Transfer (MB)", "Off-Hrs"]
        )
        ...
        for row_index, user in enumerate(rows):
            values = [
                user["user"], str(user["if_score"]), str(user["ocsvm_score"]),
                f'{user["risk_score"]}%', user["risk_level"], str(user["avg_files"]),
                f'{user["avg_transfer"]} MB', str(user["off_hours"]),
            ]
